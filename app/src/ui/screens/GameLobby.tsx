...
const tabsStyles = {
  trigger:
    "h-full bg-primary/50 data-[state=active]:bg-secondary/50 truncate rounded-b-none border-primary border-[1px] data-[state=active]:border-b-transparent disabled:bg-primary/25 sm:tracking-[0.25rem] text-xs sm:text-2xs lg:text-base",
  content:
    "h-full bg-primary mt-0 bg-secondary/50 border-x-[1px] border-primary",
};
...
const GameTablePanel = ({ children }: { children: ReactNode }) => (
  <div className="w-full sm:w-7/12 z-0 flex flex-col p-4">{children}</div>
);
...
const GameTable = ({ gameMode }: { gameMode: Mode }) => {
  ...
  const [games, setGames] = useState<{
    [key: number]: ComponentValue<Schema, unknown>;
  }>({});
  ...
  const [recentGame, setRecentGame] = useState<ComponentValue<
    Schema,
    unknown
  > | null>(null);
  ...
  useMemo(() => {
    defineEnterSystem(
      ...
      function ({ value: [game] }: ComponentUpdate) {
        setGames((prevTiles: ComponentValue<Schema, unknown>) =>
          game ? { ...prevTiles, [game.id]: new GameClass(game) } : prevTiles,
        );
      },
    );
    defineSystem(
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <Tabs
        defaultValue={gameMode.value.toLowerCase()}
      ...
      >
        <TabsList className="flex-shrink-0 p-0 bg-transparent justify-start">
          {tabs.map((tab, index) => (
            <Fragment key={index}>
              <div
                className={`h-4 self-end border-b-[1px] border-primary ${index === tabs.length - 1 ? "flex-grow" : "w-1 sm:w-6"}`}
              />
              <TabsTrigger
              ...
              {index === tabs.length - 1 && (
                <div
                  className={`h-4 self-end border-b-[1px] border-primary w-6 hidden sm:block`}
                />
              )}
            </Fragment>
          ))}
        ...
        <div className="flex-1 overflow-hidden">
          {tabs.map((tab) => (
            <TabsContent
              key={tab}
              value={tab}
              className={`${tabsStyles.content} h-full overflow-y-auto`}
            >
              <Games games={games} />
            </TabsContent>
          ))}
        </div>
        {gameMode.value !== ModeType.Tutorial && (
...
const CreateGameSliver = ({ gameMode }: { gameMode: Mode }) => (
  <div
    className={
      "w-full flex justify-between sm:justify-end h-20 border-x-[1px] border-b-[1px] border-primary bg-secondary/50 p-4"
    }
  >
    <ProfileSheet />
