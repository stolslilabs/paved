...
:::note
Remember: characters **cannot** be placed on structures that connect directly to an existing structure that already has allegiance to a player. When increasing and contesting allegiance, characters must be placed on separate strutures, which can then be merged with subsequent tiles.
:::
