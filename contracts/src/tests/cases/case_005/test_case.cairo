...
fn test_case_005() {
    ...
    store.set_game(game);
    systems.weekly.draw(world, game.id); // RFRFFFFFR
    ...
    store.set_game(game);
    systems.weekly.draw(world, game.id); // SFRFRFFFR

    let orientation = Orientation::East;
    let x = CENTER - 1;
    let y = CENTER - 1;
    systems.weekly.build(world, context.game_id, orientation, x, y, Role::Adventurer, Spot::North);
    ...
    store.set_game(game);
    systems.weekly.draw(world, game.id); // WFFFFFFFR

    let orientation = Orientation::North;
    let x = CENTER;
    let y = CENTER - 1;
    systems.weekly.build(world, context.game_id, orientation, x, y, Role::Pilgrim, Spot::Center);
    ...
    store.set_game(game);
    systems.weekly.draw(world, game.id); // RFFFRFFFR

    let orientation = Orientation::East;
    let x = CENTER + 1;
    let y = CENTER - 1;
    systems.weekly.build(world, context.game_id, orientation, x, y, Role::Lord, Spot::Center);
