...
fn test_play_ranked_tournament_claim() {
    ...
    let x = CENTER;
    let y = CENTER + 1;
    systems.daily.build(game_id, orientation, x, y, Role::Lord, Spot::South);
    ...
    let x = CENTER;
    let y = CENTER + 1;
    systems.daily.build(game_id, orientation, x, y, Role::Paladin, Spot::South);
    systems.daily.surrender(game_id);
    ...
    let tournament_id = TournamentImpl::compute_id(time, 604800);
    let rank = 1;
    systems.daily.claim(tournament_id, rank);
