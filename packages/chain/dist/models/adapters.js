import { Game, Player, Builder, Character, Tile, Tournament, } from "@paved/game-core";
/** Convert raw Torii entity to Game instance */
export function toGame(raw) {
    const data = {
        id: Number(raw.id ?? 0),
...
export function toCharacter(raw) {
    const data = {
        game_id: Number(raw.game_id ?? 0),
        player_id: raw.player_id ?? 0,
...
export function toTournament(raw) {
    const data = {
        id: Number(raw.id ?? 0),
        prize: raw.prize ?? 0,
