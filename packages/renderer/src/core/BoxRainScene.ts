...
export class BoxRainScene {
  ...
  async init(canvas: HTMLCanvasElement, basePath = ""): Promise<void> {
    ...
    for (let i = 0; i < BOX_COUNT; i++) {
      ...
      try {
        ...
        model.position.set(
          ...
          INITIAL_Y_MIN + Math.random() * (INITIAL_Y_MAX - INITIAL_Y_MIN),
          SPREAD_Z_MIN + Math.random() * (SPREAD_Z_MAX - SPREAD_Z_MIN)
        );
        model.rotation.set(
